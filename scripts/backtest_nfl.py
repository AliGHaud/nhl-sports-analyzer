"""
Backtest harness for the NFL lean engine.

Usage (default full season window):
  python scripts/backtest_nfl.py

Custom dates:
  python scripts/backtest_nfl.py --start 2024-09-05 --end 2025-01-07
"""
...
def _lean_scores_nfl(home, away, games, game_date=None):
    ...
    home_score, away_score, _ = lean_matchup(
...
def backtest(
    ...
    min_edge: float = MIN_EDGE_FAVORITE,
    min_edge_dog: float = MIN_EDGE_UNDERDOG,
    min_prob: float = MIN_MODEL_PROBABILITY,
    stake: float = 1.0,
    processes: int = 1,
    quiet: bool = False,
) -> Dict[str, Any]:
    ...
    results_path = f"nfl_backtest_results_{start_date}_to_{end_date}_edge_{min_edge:.3f}.json"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Backtest NFL lean engine.")
    parser.add_argument("--start", default="2024-09-05", help="Start date YYYY-MM-DD")
    parser.add_argument("--end", default="2025-01-07", help="End date YYYY-MM-DD")
    parser.add_argument("--odds", help="Path to clean odds CSV (optional)")
    parser.add_argument(
        "--min-edge",
        type=float,
        default=MIN_EDGE_FAVORITE,
        help="Minimum model edge (prob - market) required to count a bet (default 0.05)",
    )
    parser.add_argument(
        "--min-prob",
        type=float,
        default=MIN_MODEL_PROBABILITY,
        help="Minimum model probability threshold required to count a bet (default 0.5)",
    )
    parser.add_argument(
        "--min-edge-dog",
        type=float,
        default=MIN_EDGE_UNDERDOG,
