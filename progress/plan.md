...
- Injuries: Rotowire JSON injury feed with TTL cache; `/nhl/injuries` endpoint; matchup responses include injuries (per team) + source/timestamp; UI card shows injuries. Injury entries now include `important` flag (softer thresholds: skater gp>=5 & toi>12; goalie gp/starts>=5 & sv%>=0.895) when stats provided, and injuries now adjust lean scores (goalie -0.6, key skaters -0.25 each, depth -0.1; capped) with reasons. Injuries are included in snapshots.
